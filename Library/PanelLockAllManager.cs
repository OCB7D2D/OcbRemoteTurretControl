...
static class PanelLockAllManager
{
		...
		string _customUi = null)
	{
		...
		foreach (KeyValuePair<TileEntity, int> lockedTileEntity in locked)
		{
			...
			{
				//locked[tileEntity] = _entityIdThatOpenedIt;
				Log.Out("We already have a look somehow, nothing to do?");
				Log.Out(" lock {0} by {1}", lockedTileEntity.Value, lockedTileEntity.Key);
				return;
			 }
		}
