...
{
    public IEnumerator DebugTick()
    {
        while (true)
        {
            Log.Out("Debug Block {0}, Kamera {1}, CurrentCam {2}, RemoteTurrets {3}, ControlPanels {4}",
                Block, Kamera, CurrentCam, RemoteTurrets.Count, ControlPanels.Count);
    ...
    public RemoteTurretPanel(WorldBase world, int clrIdx, Vector3i pos, Transform monitor, BlockRemoteTurret block)
    {
        ...
        GameManager.Instance.StartCoroutine(DebugTick());
    }
    ...
    public void Tick(bool active)
    {
        ...
        if (Kamera == null) {
            Log.Out("Detected Kamera gone mad");
            return;
        }
        Log.Out("Ticky {0}", Texture);
        ...
        else if (isPowered)
        {
            ...
            {
                Log.Out("Detected a gone bad one");
